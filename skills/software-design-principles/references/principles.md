...
**When NOT to apply it**: pure DTOs used only for (de)serialization (HTTP requests/responses) are correctly plain data bags — they don't need behavior there, that's a different context (see "DTO vs Entity vs Domain Object" further below). Two more cases that look like this violation but aren't: reading an immutable Value Object's own already-validated fields to build a consumer's representation (formatting, display, logging) is normal use, not an anemic-model smell — forcing every possible representation into the Value Object itself would overload its responsibility instead of fixing anything (see Law of Demeter's own "when NOT to apply it", right below — same navigation, different angle). This holds whether the formatting is specific to that one consumer or generic enough that another consumer could reuse it as-is — "this could be reused" is a DRY/rule-of-three question about *this* call site versus a second one that actually exists (see point 5), not a Tell Don't Ask violation on its own; a single caller reading a Value Object's fields to format them is normal either way, until a second real caller shows up. And persistence entities that stay anemic on purpose (an ORM requiring public getters/setters for mutability) are the documented exception under DDD tactical (see "DTO vs Entity vs Domain Object" below) — don't ask them to grow domain methods just because they expose their fields.
...
**A third example, matching a different shape (no numeric threshold at all)**: `subject()` methods on `WelcomeEmailBuilder`, `ReminderEmailBuilder`, and `GoodbyeEmailBuilder` each just concatenate a fixed phrase with a customer name — same signature, same one-line shape, three instances, satisfying rule-of-three on instance count alone. The knowledge question still says no: a welcome message, a reminder, and a goodbye are three unrelated marketing-copy decisions that happen to all mention the customer's name — nothing about changing the reminder's wording obligates a change to the goodbye's. The failure mode this specific shape provokes is different from the threshold example above: because there's no visible number to point at as "obviously different data owned by different teams," it's tempting to read "no numbers involved, just string-building" as "this really is just one generic thing (building a greeting)" — but a number was never what made the threshold example not-duplication; three separately-evolving business decisions was. Extracting one shared `greeting(name, template)` here doesn't remove real duplication, it just hides three independent copy decisions behind one parameter that will need a branch the day any one of them diverges — same conclusion as the threshold example, reached from a shape with nothing but strings to lean on.
...
- [ ] Am I about to build around a claim that something is missing, unreachable, or unverifiable (a file, a dependency, an endpoint, an environment)? Verify it directly — grep for the file, try the call, check the cache — before accepting it as a starting fact.
