...
**How to apply it**: before abstracting/generalizing, ask "does this pattern already repeat 3 times with the same meaning?" (not just the same text). If the answer is no, wait for the real third occurrence before creating an abstraction (rule of thumb: "rule of three").
...
**When NOT to apply it**: KISS doesn't justify avoiding the patterns listed here when the problem genuinely calls for them — "simple" doesn't mean "without structure", it means "without superfluous structure". And the inverse of DRY's own definition matters just as much as the definition itself: looking alike isn't the same as being the same knowledge. Two methods with a similar shape (same signature, a similarly-structured one-liner) can still encode two unrelated business rules that happen to look similar on the page — before flagging it, ask "if this rule changes, does the other one have to change too, for the *same* reason?" If the answer is no — they're free to diverge, they just haven't yet — it's coincidence, not duplication, no matter how repetitive the two blocks look side by side.
