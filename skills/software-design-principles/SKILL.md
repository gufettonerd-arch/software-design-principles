...
description: Software design principles (SOLID, Value Object, Tell Don't Ask, DDD, Hexagonal, Strangler Fig, etc.) and a step-by-step procedure for extracting a flow from a legacy god class without breaking anything. Use this skill whenever you write, review, or refactor code in any project — when deciding whether to introduce an interface/abstraction, when weighing a generic catch(Exception e), when you spot magic numbers or positional tuples, when you're about to add a static/shared field or a singleton/service-scoped cache touched by concurrent requests (thread-safety, shared mutable state), or a client-side singleton/store (e.g. an Angular service, a React/Vue global store) that could keep one user's or session's data around for the next one, when you're about to re-validate data that was already checked earlier in the same flow instead of trusting the boundary that validated it, when you're touching legacy code with no tests and deciding what the first test should assert, when deciding whether to organize packages/folders by technical layer (controller/service/repository) or by business feature, when you notice the same logic or validation duplicated for the same underlying concept and not just superficially similar code, when you're chaining more than two getters/accessors to reach a value (a.getB().getC().getD()), when a growing monolith's teams keep colliding and you're weighing a modular restructuring against a full microservices split, when the same term (e.g. "Order", "Customer") means something different across teams or modules and you're deciding whether they should share one model or each own a local one (bounded context), when you're incrementally replacing a legacy module with a new implementation piece by piece instead of a big-bang cutover or no-downtime rewrite, when doing a final self-review pass on code that already works but is hard to follow, before opening a PR, or when you need to extract/isolate a method from an oversized legacy class (god class) without a big-bang rewrite. Also trigger it just for the working checklist before starting a task or before opening a PR.
---
...
- `references/god-class-extraction-playbook.md` — a 0→13 procedure for extracting a method/flow from a legacy god class (any language) without losing behavior: map dependencies, extract without deleting the old code, move the tests, remove dead code, handle shared code (Strangler Fig with an explicit completion criterion), isolate the fix, readability pass, Service/Repository/Validator separation, specific exceptions, hardening (parameterized queries, no shared mutable state), coverage, end-to-end verification, and a final fresh-eyes re-review of the finished class as a whole. Open this file when the task is exactly "extract/isolate code from an oversized legacy class".
