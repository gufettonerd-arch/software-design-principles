...
bugs a careful baseline missed, and not adding ceremony a baseline
correctly skipped. On a narrow or heavily-constrained task this doesn't
show up at all (round 3); on a real, bigger task with genuine judgment
space, baseline, with-skill, and a session given no instruction either
way all independently reach for the same correct extraction shape
(round 1, round 4) — the skill's contribution is the paper trail, not
the outcome. Two of the four rounds also surfaced the same kind of
false-positive-caller trap in a different codebase each time (a plain
`grep` misreading a non-UTF-8 file as binary; a syntactically valid but
...
- **Trigger accuracy had a real, now-fixed gap around one principle.** The `description` field is what an agent matches against your request to decide whether to consult the skill — written by hand, not run through a proper trigger-accuracy eval. A first check (8 clear-cut prompts) went 8/8. A 2026-09-08 follow-up tried the boundary that check flagged as untested (6 deliberately ambiguous prompts): triggered correctly 3/6 — including one correct *decline*, citing its own "when NOT to apply it" clause — and missed once, on a `static HashMap` cache in a concurrent app, close to a word-for-word match of principle 18's (shared state) own documented example. A same-day follow-up targeting principle 18 specifically (4 more prompts, including the almost-exact textbook case — a `SimpleDateFormat` instance field on a singleton service) found the same miss **every time: 5/5 non-triggers total** on principle-18-shaped prompts, baseline reasoning getting every one of them right regardless. Root cause: the `description` field listed cues for several principles but never mentioned concurrency, thread-safety, or shared/static state — principle 18 had no keyword surface to match against. Fixed the same day (plugin 1.2.9 → 1.3.0) by adding one. Not yet reverified against live routing — an installed plugin doesn't pick up a repo change until it's explicitly synced (see the next bullet), so confirming this actually changes trigger behavior needs a fresh boundary-test round after that sync. See [`benchmarks/README.md`](benchmarks/README.md) for the full breakdown.
